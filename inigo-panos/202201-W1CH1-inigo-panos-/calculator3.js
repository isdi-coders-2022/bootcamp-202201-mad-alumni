...
tdNumber.forEach(td => {
    td.addEventListener('click', () =>{
        ...
        screen.textContent = currentValue.join('');
        //actualizarNumeros(currentValue);
...
function setOperation()
{
    ...
    if (/*resultado == 0 &&*/ paso2 == false) //Si no hay un resultado anterior, se ejecuta normal
    {
        checkOperation(prueba);
        console.log('1');
        paso2 = true;
    }
    else if (paso2) //Si hay un resultado anterior, el resultado tiene que ser num1
    {
        console.log('2');
        ...
        previousNumber.textContent = num1 + ' ' + previousOperation + ' ' + num2;
        num1 = resultado;
        //resultado = 0;
        num2 = 0;
    }
...
{
    //Aquí se guarda num1
    if (resultado === 0) //Si no hay un resultado anterior, es decir, es la primera vez que se introduce un número
    {
    ...
    else //Si hay un resultado anterior, es decir, después de una operación se busca hacer otra. 
    {
...
function Operate(operar) //Resultado, PASO 4
{
    ...
    //previousNumber.textContent = lastOperation;
    paso2 = false;
}
